...
Each dataspace participant has its own keypair and a dedicated wallet account within the single platform-hosted walt.id deployment. The platform cannot sign on behalf of a company without access to that company's wallet credentials. (Future: companies may bring their own wallet deployment — see G4.)

**G2 — Walt.id as the Wallet Infrastructure**
All wallet-related operations (key generation, VC issuance, VC storage, VP signing) are delegated to the walt.id Community Stack. The backend is a coordinator, not a key holder.

**G3 — Platform Operator Wallet**
The platform operator (operating company) has its own dedicated wallet for trust anchor responsibilities: issuing Membership VCs to participants and holding the operator's own Gaia-X credentials.
...
**US-5** — As a company admin, when I complete onboarding, a Gaia-X LegalParticipant VC is issued and stored in *my company's* wallet, signed by *my company's* own key.
...
- **R1.** When a company is onboarded via `POST /api/companies`, the backend must automatically:
  ...
  d. Generate an RSA-2048 key in the wallet (used for Gaia-X GXDCH compliance submissions)
...
- **R7.** All Verifiable Credentials must be signed using the issuing company's own key in their walt.id wallet — not the platform key.

- **R8.** Gaia-X compliance VCs (LegalParticipant, RegistrationNumber, TermsAndConditions) must use the company's RSA-2048 key with the `x5c` header as required by GXDCH.

- **R9.** All other VCs (Membership, Ownership, etc.) must use the company's Ed25519 key with `EdDSA` algorithm.
...
- **R12.** Signed VCs must be stored in the company's walt.id wallet as the primary store.
...
- **R19.** The `VPSigner` service must be refactored to delegate all signing operations to the appropriate company's walt.id wallet.

- **R20.** Key export (private key leaving the wallet) must never happen. The backend interacts with wallet APIs — it does not hold private keys.
...
**OQ-1 — GXDCH x5c Header with Walt.id**
The Gaia-X compliance endpoint (GXDCH) requires RSA JWTs with an `x5c` header containing an X.509 certificate chain. The walt.id issuer API may not support adding custom JOSE header fields like `x5c`.
- *Options to investigate:*
  a. Check if walt.id issuer API supports `headers` override in request body
  b. Use a hybrid approach: sign the JWT via wallet signing endpoint, then add `x5c` in backend (requires reviewing JWT structure)
  c. Keep a platform-level signing step for GXDCH only (not per-company RSA), with per-company Ed25519 for all other credentials
